...
for thread in "${THREADS[@]}"; do
    for i in {1..5}; do
      ...
      /usr/bin/time -f "$thread,$thread,$i,%e" ./philo "$thread" >>measures/measure_philo.csv 2>&1                    # s = silent (do not print commands in terminal), j = number of jobs(threads)
      make -s clean
    done
done
...
for thread in "${THREADS[@]}"; do
    if [ "$thread" != 1 ]; then    #comme que des thread pair, on pourrait le rerirer
      ...
      for i in {1..5}; do
            make rw -j "$thread" -s
            /usr/bin/time -f "$r,$w,$thread,$i,%e" ./rw "$r" "$w" >>measures/measure_rw.csv 2>&1                    # s = silent (do not print commands in terminal), j = number of jobs(threads)
            make -s clean
      done
    fi
done
