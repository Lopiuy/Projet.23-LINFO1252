...
def customization(ax, labels, y, vp, problem):
    ...
    elif problem == 'tas': title += " par l'algorithme test-and-set"
    elif problem == 'ttas': title += " par l'algorithme test-and-test-and-set"
...
def customization_dual(ax, labels, vp, color1, color2, problem=''):
    ...
    ax.set_ylabel("Temps d'exécution " + "$[s]$", fontsize=12)
    title = "Diagramme en violon du problème de synchronisation\n"
    if problem == 'tas': title += " par l'algorithme test-and-set"
    elif problem == 'ttas': title += " par l'algorithme test-and-test-and-set"
    ax.set_title(title, fontsize=14)
...
def plot_dual(data1, data2, threads, nthreads):
    ...
    labels = [str(threads[i]) for i in range(nthreads)]
    y1 = data1; y2 = data2
    means1 = np.mean(y1, axis=1); means2 = np.mean(y2, axis=1)
    stds1 = np.std(y1, axis=1); stds2 = np.std(y2, axis=1);

    vp1 = ax[0].violinplot(y1, showmeans=True)
    ax[0].scatter(np.arange(1, len(labels) + 1), means1+stds1, 125*np.ones(nthreads), marker='_', lw=1, color='saddlebrown', label="écarts-types")

    vp2 = ax[1].violinplot(y2, showmeans=True)
    ax[1].scatter(np.arange(1, len(labels) + 1), means2+stds2, 125*np.ones(nthreads), marker='_', lw=1, color='saddlebrown', label="écarts-types")
...
if len(sys.argv) == 2:
    try:
        ...
        elif (sys.argv[1].__contains__("_tas")): plot(toplot, threads, nthreads, problem='tas')

        elif (sys.argv[1].__contains__("ttas")): plot(toplot, threads, nthreads, problem='ttas')

        else : print("Error: Invalid arguments were given")
    except:
